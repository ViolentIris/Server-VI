...
function s.filter(c)
	return c:IsSetCard(0x19e) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
...
function s.cfilter(c)
	return c:IsFaceupEx() and c:IsSetCard(0x19b) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToDeck()
		and not c:IsCode(id)
end
